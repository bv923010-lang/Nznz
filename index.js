...
function loadCommands() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (!cmd.run && !cmd.onCall) { fail++; continue; }
...
async function startBot(models) {
  ...
  login({ appState: appstate, ...global.config.FCAOption }, async (err, api) => {
    ...
    const handleCommand  = require("./includes/handle/handleCommand")(ctx);
    const handleEvent    = require("./includes/handle/handleEvent")(ctx);
    const handleReaction = require("./includes/handle/handleReaction")(ctx);
    ...
    api.listenMqtt(async (err, event) => {
      ...
      try {
        switch (event.type) {
          ...
          case "message_reply":
          case "message_unsend":
            // ── handleReply ROUTER (fixed) ──────────────────────────
            if (routeHandleReply(api, event, models)) break;
...
// ══════════════════════════════════════════════════════
//  handleReply ROUTER  (the "dead listener" fix)
//  When a user replies to a bot menu/dashboard message,
//  we look up the registered handler and call it.
// ══════════════════════════════════════════════════════
function routeHandleReply(api, event, models) {
  const { messageReply, senderID, threadID } = event;
  if (!messageReply) return false;

  const repliedMid = messageReply.messageID;
  const idx = global.client.handleReply.findIndex(
    h => h.messageID === repliedMid
  );
  if (idx === -1) return false;

  const entry = global.client.handleReply[idx];

  // Remove after routing (one-shot) unless cmd opts into persistence
  const cmd = global.client.commands.get(entry.commandName);
  if (!cmd?.config?.persistReply) {
  ...
  // Sandbox the reply handler too
  (async () => {
    try {
      if (typeof entry.handler === "function") {
        await entry.handler(unifiedCtx);
      } else if (cmd?.handleReply) {
        await cmd.handleReply(unifiedCtx);
      }
    } catch (e) {
      log.error(`handleReply [${entry.commandName}] ত্রুটি: ${e.message}`);
      _notifyAdmin(`handleReply ত্রুটি [${entry.commandName}]: ${e.message}`);
    }
  })();

  return true; // consumed — don't process as normal command
}
