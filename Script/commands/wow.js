...
module.exports.config = {
  ...
  description: "SAD VIDEO STREAMS - BYPASS 429",
...
module.exports.run = async ({ api, event }) => {
  try {
    ...
    api.sendMessage("Please wait master, fetching video... ⏳", event.threadID, event.messageID);

    let response;
    try {
      // প্রথম চেষ্টা: সরাসরি ইমেগুর থেকে ডাউনলোড
      ...
      if (e.response && e.response.status === 429) {
        // দ্বিতীয় চেষ্টা: ৪২৯ এরর আসলে প্রক্সি লিংক দিয়ে ইমেগুর বাইপাস করা
        const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(randomLink)}`;
        response = await axios({
          method: "GET",
          url: proxyUrl,
          responseType: "stream"
    ...
    writer.on("error", (err) => {
      api.sendMessage(`ফাইল তৈরি করতে সমস্যা: ${err.message}`, event.threadID, event.messageID);
    });

  } catch (error) {
    api.sendMessage(`দুঃখিত মাস্টার, সার্ভার ওভারলোডের কারণে ভিডিওটি আনা যায়নি। আবার চেষ্টা করুন।`, event.threadID, event.messageID);
  }
};
